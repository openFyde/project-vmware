...
IUSE="${IUSE_VIDEO_CARDS}
	...
	cheets_user cheets_user_64 arc-64bit-only"
...
# Unable to do Vulkan 1.3 for upgrade Intel devices due to incomplete ndk translation
force_vulkan11() {
	use vulkan && use video_cards_intel && use !arc-64bit-only
}
...
src_prepare() {
  ...
  if [[ ${PV} != 9999* && -n ${SRC_PATCHES} ]]; then
	...
	if force_vulkan11 ; then
		einfo "Downgrade to Vulkan 1.1 for incomplete ndk translation."
		eapply "${FILESDIR}/CHROMIUM-Revert-venus-enable-Vulkan-1.3-for-Android-13.patch"
	fi
...
multilib_src_install_all() {
	...
	if use vulkan; then
		...
		if (( ARC_PLATFORM_SDK_VERSION < 33 )) || force_vulkan11 ; then
			doins "${FILESDIR}/android.hardware.vulkan.version-1_1.xml"
		else
			doins "${FILESDIR}/android.hardware.vulkan.version-1_3.xml"
		fi
