...
CROS_WORKON_COMMIT="70d0e8cb7b2347427161bff63e4d951684342b5c"
CROS_WORKON_TREE="874c37b1b1f067684e73552a90e3b1c29b44aa44"
...
src_prepare() {
    ...
    epatch ${FILESDIR}/patches/svga_format.patch
    if use debug; then
       epatch ${FILESDIR}/patches/debug_printf_for_arc.patch 
    fi
...
multilib_src_configure() {
	...
	# Configurable DRI drivers
		driver_enable video_cards_llvmpipe swrast
	...
	if use cheets; then
        ...
        #export XCB_DRI2_CFLAGS="-I${ROOT}/usr/include"
        #export XCB_DRI2_LIBS="-L${ROOT}/usr/lib64 -lxcb-dri2"
		...
		#
        #export GLPROTO_CFLAGS="-I${ROOT}/usr/lib64/include"
        #export GLPROTO_LIBS="-L${ROOT}/usr/lib64"
        #export DRI2PROTO_CFLAGS="-I${ROOT}/usr/lib64/include"
        #export DRI2PROTO_LIBS="-L${ROOT}/usr/lib64"
	fi
...
multilib_src_install_cheets() {
	...
	if use gallium; then
        ...
        if use video_cards_vmware; then
            newexe $(get_libdir)/gallium/vmwgfx_dri.so vmwgfx_dri.so
        fi
	fi
