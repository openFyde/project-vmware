...
src_prepare() {
	eapply -p2 "${PATCHES[@]}"
#  eapply "${FILESDIR}/001-fix-configure-issue.patch"
	eapply_user
	eautoreconf
}
...
src_configure() {
  ...
  cros_use_gcc
	CUSTOM_PAM_CPPFLAGS="-I${ROOT}/usr/include" \
  CUSTOM_GRPC_CPPFLAGS="-I${ROOT}/usr/include" \
  CUSTOM_CURL_CPPFLAGS="-I${ROOT}/usr/include" \
   econf "${myeconfargs[@]}"
}
